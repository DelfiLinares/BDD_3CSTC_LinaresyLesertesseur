...
-- Crear una Stored Function que reciba un id de cliente e indique si es o no frecuente.
-- Es frecuente si realizó al menos un 5% de las compras totales, en los últimos 6 meses.

delimiter //
create function frecuenciaCliente(idCliente varchar(20)) returns text deterministic 
begin 
	declare comprasTotales int default 0;
	declare porcentajeCompras int default 0;
	select count(idPedido) into comprasTotales from pedido;	
	select (5*count(idPedido))/100 into porcentajeCompras from pedido;
    
    if (comprasTotales<porcentajeCompras and pedido.fecha between current_date() 
		and subdate(current_date(), interval 6 month)) then
		return "Cliente NO frecuente";
        else
        return "Cliente frecuente juju";
        end if;
end// 
delimiter ;

select frecuenciaCliente("2");

select count(idPedido) from pedido;
select (5*count(idPedido))/100 from pedido;

select @porcentajeCompras; 



select count(idPedido) , (5*count(idPedido))/100 from pedido
where fecha between current_date() and subdate(current_date(), interval 6 month)
;	
	
select subdate(current_date(), interval 6 month);    

-- EJERCICIO 2------------------------------------------------------------------------

-- 4- Crear una Stored Function que reciba un id de cliente y devuelva la cantidad de 
-- pedidos pendientes de pago de ese cliente.

delimiter //
create function pendientes(idCliente varchar(20)) returns int deterministic 
begin 
	declare cantidad int default 0;
	set cantidad = (SELECT count(idPedido));
    
    select count(idPedido) from pedido join estado on  
    estado.idEstado = pedido.Estado_idEstado;
    
    if (estado.nombre = "Por enviar") then
    return cantidad;  
    end if;
...
delimiter//
create function cantEntradas (id int) returns int deterministic
begin
	declare cantidad int;
    select sum(cantEntradas) into cantidad from compras 
...
delimiter ;
-- ------------------------------------------------------------------------------------------    
create function calcularPuntos (idC int) returns float deterministic    
    begin
		declare puntos float;
        set puntos = 0.25 * (select sum(valorEntrada * cantEntradas))
        from funcion join compra on idfuncion = Funcion_idfuncion where
        Cliente_idcliente = idC and timestampdiff(month,fecha,current_date()= 1);
        return puntos;
    end//    
delimiter ;

update cliente set puntos = calcularPuntos(1);
update cliente set puntos =calcularPuntos(1) where idcliente = idC;

-- ----------------- EJERCICIOS PROCEDIMIENTOS Y CURSORES --------------------------- --
#1 Crear un SP que liste todos los productos que tengan un precio de compra 
-- mayor al precio promedio y que devuelva la cantidad de productos 
-- que cumplan con esa condición.

delimiter //
create procedure listaProductos (in idproducto int)
begin 
	declare cantidadproductos int default 0;
    set cantidadproductos = (select count(productCode) from products);
